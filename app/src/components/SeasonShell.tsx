...
export function SeasonShell({ children, dark = true }: { children: (snap: SeasonSnapshot) => React.ReactNode; dark?: boolean }) {
  ...
  if (error) return <main className={`min-h-[calc(100vh-56px)] ${cls} p-8 font-data text-[13px]`}>RPC error: {error.message.split("\n")[0]}</main>;
  if (!snapshot || isLoading) return <main className={`min-h-[calc(100vh-56px)] ${cls} p-8 text-mine-muted`} data-testid="loading">Reading the mine…</main>;
  return <main className={`min-h-[calc(100vh-56px)] ${cls}`}>{children(snapshot)}</main>;
}
