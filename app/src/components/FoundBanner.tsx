...
export function FoundBanner({ blockIdx, fragments, fragPerToken, onDismiss }: { blockIdx: number; fragments: bigint; fragPerToken: bigint; onDismiss: () => void }) {
  return (
    <div className="bg-signal text-[#062126] px-4 md:px-8 py-3.5 flex items-center gap-4 md:gap-6" data-testid="found-banner">
      <Gem size={36} />
      <div className="font-display uppercase tracking-[0.02em] text-[22px] font-semibold">Block {blockIdx + 1} found · {TICKERS[blockIdx]}</div>
      ...
      <Link href="/claim" className="ml-auto h-9 px-4 rounded-sm bg-[#062126] text-signal inline-flex items-center text-[14px] font-semibold">Claim {TICKERS[blockIdx]}</Link>
