...
export function MineralScene({ kind, value, label }: { kind: SceneKind; value: number; label: string }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(async ([entry]) => {
      ...
      try {
        ...
        // The original vector illustration remains visible when WebGL is unavailable.
      }
    }, { rootMargin: "250px" });
  ...
  return <div ref={host} className={`lp-scene lp-scene-${kind} ${ready ? "is-ready" : ""}`} role="img" aria-label={label}>
