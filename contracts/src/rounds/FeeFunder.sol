...
/// @title FeeFunder – the Pons tax recipient that turns ETH into the running round's pot
...
///         only accepts a configured pool as `msg.sender` and only pays WETH, so a foreign caller can
///         never pull funds. The flusher's `minOut` is the slippage guard; the owner is the mine's
...
contract FeeFunder is IFeeFunder, ReentrancyGuard {
    ...
    /// @dev The Pons tax arrives as plain ETH transfers.
    receive() external payable {}

    /// @inheritdoc IFeeFunder
    function flush(uint256[] calldata minOut) external nonReentrant returns (uint256[] memory tokensOut) {
        ...
        emit Flushed(msg.sender, ethIn, tokensOut, IRoundMine(mine).currentRound());
    }

    /// @dev Uniswap v3 pays the pool through this callback; only a configured pool may call it and
    ///      only the WETH side is ever paid, so no other caller can move funds out of here.
