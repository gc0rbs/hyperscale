/**
 * Fee flusher (docs/13 §2 "Funding"): Pons V2 credits the creator fee recipient (the FeeFunder) in
 * its fee escrow. Every tick this loop simulates a flush from the flusher key: the funder runs its
 * collect calls (sweep the pending fees into the escrow, claim them), wraps the ETH, splits the WETH
 * across the four Stock Tokens on their WETH pools and funds the running round, in one transaction.
 * The simulation is the quote; the real send carries a haircut of it as the slippage bounds. Runs
 * from the flusher key, which holds nothing but gas.
 *
...
async function tick() {
  ...
  const [held, halted, allowed, gas, legCount, collects] = await Promise.all([
  ...
  if (halted) { alert("page", `mine is HALTED; ${formatEther(held)} ETH/WETH waits on the FeeFunder for the owner to sweep`); return; }
  if (collects === 0n) alert("info", `FeeFunder has no Pons collect calls yet (rounds-admin set-source after the token launch); flushing only what is pushed here`);

  // Quote: simulate with zero bounds. The simulation sweeps, claims from the escrow and swaps at
  // current prices, so its result is exactly what a send would do right now.
  ...
  try {
    ...
    const [a, b] = sim.result as [bigint, bigint[]];
    wethIn = BigInt(a); outs = b.map(BigInt);
  } catch (e) {
    const msg = (e as Error).message;
    if (/NothingToFlush/.test(msg)) { alert("info", `nothing to flush (held ${formatEther(held)} ETH/WETH, nothing claimable); waiting`); return; }
  ...
  const desc = `${formatEther(wethIn)} WETH → ` + outs.map((o, i) => `${dep.symbols[legs[i].stock]}≥${formatEther(minOut[i])} (quote ${formatEther(o)})`).join(" ");
